...
router.get("/", async (req, res) => {
    try {
        ...
        // If search query exists, filter by class name OR class invite code
        if (search) {
        ...
        if (subject) {
            const subjPattern = `%${String(subject).replace(/[%_]/g, '`\\$&')}%`;
            filterConditions.push(ilike(subjects.name, subjPattern));
        }
